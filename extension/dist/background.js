...
const AGENT_BRIDGE_KEEPALIVE_MS = 2e4;
const SIDEBAR_RUNTIME_STATE_KEY = "opencli_sidebar_runtime_v1";
const emptySidebarTarget = () => ({
  status: "unbound",
  tabId: null,
  windowId: null,
  title: "",
...
function notifySidebarTarget() {
  notifySidePanel({ type: "sidebarTargetStatus", target: sidebarTarget });
}
function setSidebarTarget(next) {
  sidebarTarget = next;
  persistSidebarRuntimeState();
  notifySidebarTarget();
}
async function recoverSidebarRuntimeState() {
  try {
    const raw = await chrome.storage.session.get([SIDEBAR_RUNTIME_STATE_KEY]);
    const stored = raw?.[SIDEBAR_RUNTIME_STATE_KEY];
...
async function bindSidebarTarget(tabId, windowId) {
  ...
  if (tab.windowId !== windowId) {
    return {
      id: "sidebar_bind",
      ok: false,
      errorCode: "bound_tab_window_mismatch",
  ...
  setSidebarTarget({
    ...
    title: tab.title ?? "",
    url: tab.url ?? "",
    bindingEpoch: sidebarTarget.bindingEpoch + 1,
    taskId: null,
    taskBindingEpoch: null
  });
...
async function unbindSidebarTarget() {
  if (sidebarTarget.status === "busy") {
    return {
      id: "sidebar_unbind",
      ok: false,
...
function completeSidebarTask(replyTaskId, replyBindingEpoch) {
...
function clearAgentBridgeReconnect() {
...
function scheduleAgentBridgeReconnect() {
  ...
  const delay = Math.min(3e4, 1e3 * 2 ** Math.min(agentBridgeReconnectAttempts, 5));
...
function connectAgentBridge(config, force = false) {
  ...
  const active = agentBridgeWs?.readyState === WebSocket.OPEN || agentBridgeWs?.readyState === WebSocket.CONNECTING;
  ...
  socket.onopen = () => {
    ...
    const registration = {
      ...
      metadata: {
        ...
        device_id: config.deviceId || void 0
      }
    };
...
async function reconnectAgentBridgeFromStorage(force = false) {
  const config = await loadRuntimeConfig();
  connectAgentBridge(config, force);
}
function sendAgentBridgePayload(payload, displayText) {
  ...
  if (cmd.targetPolicy === "bound-only") {
    ...
    if (session !== "sidebar") {
      return {
        id: cmd.id,
        ok: false,
        errorCode: "sidebar_session_required",
